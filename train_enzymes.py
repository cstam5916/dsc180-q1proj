...
def main():
    dataset = pyg.datasets.TUDataset(root='data/TUDataset', name='ENZYMES', force_reload=True).to(device)

    dataset = dataset.shuffle()
    train_dataset = dataset[:500]
    val_dataset = dataset[500:550]
    ...
    for model in [gcn, gin, gat]: # add gat
        ...
        for epoch in tqdm(range(300)):
            ...
            if((len(val_loss_list) >= 30) and val_loss > val_loss_list[-1]):
                patience_counter += 1
